...
import {
  DxfDecodeEngineProxy,
  type DxfDecodeRequest,
} from "./DxfDecodeEngineProxy.js";
...
class FakeWorker implements WorkerLike {
  ...
  readonly posted: {
    readonly envelope: RequestEnvelope<DxfDecodeRequest>;
    readonly transfer: readonly Transferable[];
  }[] = [];

  postMessage(message: unknown, transfer: readonly Transferable[] = []): void {
    this.posted.push({
      envelope: message as RequestEnvelope<DxfDecodeRequest>,
      transfer,
    });
  }
...
describe("DxfDecodeEngineProxy", () => {
  ...
  it("constructs the worker only once, reused across repeated calls", () => {
    ...
    const proxy = new DxfDecodeEngineProxy(createWorker);
  ...
  it("sends the bytes, transferring the buffer", () => {
    const worker = new FakeWorker();
    const proxy = new DxfDecodeEngineProxy(() => worker);
  ...
  it("resolves with the DecodedModel the worker responds with", async () => {
    const worker = new FakeWorker();
    const proxy = new DxfDecodeEngineProxy(() => worker);
    const model: DecodedModel = { ...emptyModel, metadata: { layer: "0" } };
  ...
  it("propagates a worker-reported failure", async () => {
    ...
    const result = proxy.transform(new Uint8Array([1]));
    worker.respond({ id: 0, ok: false, error: "invalid-dxf" });

    await expect(result).rejects.toThrow("invalid-dxf");
  });
});
